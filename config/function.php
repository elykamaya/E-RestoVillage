...
function kd_cust_pemesanan()
{
	// global $mysqli;
	global $db;
	$sql = $db->query("SELECT MAX(RIGHT(kd_customer,3)) AS kd_customer
							FROM customers");
	// $m = mysqli_fetch_assoc($sql);
	$m = $sql->fetch();

	$no = 1;
	if ($m['kd_customer'] <> NULL) {
		//$kd = number_format($m['kd_customer'], 0) +1;
		//if (strlen($kd) == 1) {
			//$no = "CUS" . "0000" . $kd;
		//} elseif (strlen($kd) == 2) {
			//$no = "CUS" . "000" . $kd;
...
function kd_trans_pemesanan()
{
	...
	$sql = $mysqli->query("SELECT MAX(RIGHT(kd_transaksi,5)) AS notrans
							FROM t_pemesanan WHERE tgl = '" . date('Y-m-d') . "'");
	$m = mysqli_fetch_assoc($sql);
